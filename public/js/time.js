...
btnConfiguration.addEventListener('click', () => {
    ...
    if (screen.width > 1024) {
        timerOptions.style.animation = 'opacity 1s forwards';
    } else {
        timerOptions.style.animation = 'opacity 1s forwards';
    };
    document.querySelector('body').style.overflow = 'hidden';
    setTimeout(() => {
        options.style.animation = 'opacity .6s forwards';
    }, 1100);
    setTimeout(() => {
        song.style.animation = 'opacity .6s forwards';
    }, 1200);
    setTimeout(() => {
        buttonsOptions.style.animation = 'opacity .6s forwards';
    }, 1300);
});

const exitTimerOptions = () => {
    setTimeout(() => {
        options.style.animation = 'removeOpacity .3s forwards';
    }, 100);
    setTimeout(() => {
        song.style.animation = 'removeOpacity .3s forwards';
    }, 200);
    setTimeout(() => {
        buttonsOptions.style.animation = 'removeOpacity .3s forwards';
    }, 300);
    setTimeout(() => {
        if (screen.width > 1024){
            timerOptions.style.animation = 'removeOpacity 1s forwards';
        } else {
            timerOptions.style.animation = 'removeOpacity 1s forwards';
        };
    }, 500);
    setTimeout(() => {
        timerOptions.style.transform = 'translateX(-3000px)';
    }, 1600);
...
buttonDecrementHours.addEventListener('click', () => {
    if (!hoursValue == 0) {
        hoursValue--;
        if (hoursValue < 10) {
            hoursOptions.value = ("0" + hoursValue).slice(-2);
...
buttonDecrementMinutes.addEventListener('click', () => {
    if (!minutesValue == 0) {
        minutesValue--;
        if (minutesValue < 10) {
            minutesOptions.value = ("0" + minutesValue).slice(-2);
...
buttonDecrementSeconds.addEventListener('click', () => {
    if (!secondsValue == 0) {
        secondsValue--;
        if (secondsValue < 10) {
            secondsOptions.value = ("0" + secondsValue).slice(-2);
...
const startTimer = () => {
    ...
    if (htimer == 0 && mtimer == 0 && stimer == 0) {
        ...
        document.querySelector('body').style.overflow = 'hidden';
        setTimeout(() => {
            finishedTimerIcon.style.animation = 'opacity .5s forwards';
        }, 1200);
        setTimeout(() => {
            finishedTimerText.style.animation = 'opacity .5s forwards';
        }, 1300);
        setTimeout(() => {
            finishedTimerButton.style.animation = 'opacity .5s forwards';
        }, 1400);
...
finishedTimerButton.addEventListener('click', () => {
    ...
    setTimeout(() => {
        finishedTimerText.style.animation = 'removeOpacity .4s forwards';
    }, 100);
    setTimeout(() => {
        finishedTimerIcon.style.animation = 'removeOpacity .4s forwards';
    }, 200);
    setTimeout(() => {
